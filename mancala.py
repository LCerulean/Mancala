...
def rules():
    display()
    print("RULES:\n")
    print("The board consists of 12 pits, each with 4 stones, and 2 empty bowls.\nYour pits are the 6 on the bottom and your bowl is above your name.")
    time.sleep(5)
    print("\nOn your turn you will pick one of your 6 pits.\nThe stones will be redistributed, 1 per pit, counter-clockwise.\nIf you pass your bowl, a stone goes in there too.")
    time.sleep(5)
    print("\nIf your last stone lands in your bowl, you get a free turn!\nYou might be able to do this multiple times in a row if you plan well.")
    time.sleep(5)
    print("\nIf your last stone lands in one of your empty pits, you take the opposite pit’s stones!\nThis only works if your opponent has stones in the opposite pit.\nThose stones are then auto-redistributed around the board like a regular turn.")
    time.sleep(5)
    print("\nThe game ends when one side does not have any stones left in their 6 pits.\nThe final score counts both your bowl and any stones you have left in your pits.\nBe careful not to run out of stones in your pits if your opponent still has a lot in theirs!")
    time.sleep(6)
    print("\nThat's it, have fun!")
...
#gets the pit selection from player and the stone count of said pit
def select_pit_take_stones(who_has_turn):
    stones = 0
    if who_has_turn == player:
        while stones == 0:
            pit = input("\nPick a pit: ")
            #gets stone count or gives error message if out of range of player pits
            try:
                pit = int(pit)
                if pit > 6 or pit < 1:
    ...
    time.sleep(1)
    if who_has_turn == player:
        print(f"\n{who_has_turn} takes {stones} stones from pit {pit}.")
    else:
        print(f"\n{who_has_turn} takes {stones} stones from pit {str(int(pit - 7))}.")
    time.sleep(1)
    print("\nRedistributing stones and updating gameboard...")
    time.sleep(1)
    return pit, stones


#takes pit chosen by player/computer and updates pits dict after redistributing
def redistribute_stones(who_has_turn, pit, stones):
    pits.update({pit:0})
    ...
    #distributes the stones counter-clockwise, adding to owner's bowl if passed
    for stone in range(stones):
        if pit - 1 < 0 or pit - 1 == not_my_bowl:
            pit = 13
            stones_in_pit = pits[pit]
            stones_in_pit += 1
            pits.update({pit:stones_in_pit})
        else:
            pit -= 1
            stones_in_pit = pits[pit]
            stones_in_pit += 1
...
def play_game():
    ...
    while game_over == False:
        ...
        #makes move and stones are redistributed
        while pit == bowl and game_over == False:
            move = select_pit_take_stones(who_has_turn)
            pit = move[0]
            stones = move[1]
            pit = redistribute_stones(who_has_turn, pit, stones)
            display() 

            #checks if last landing pit qualifies for taking oppenent's stones
            if pit in pits_owned and pits[pit] == 1 and pits[op_pits[pit]] > 0:
                ...
                while pit in pits_owned and pits[pit] == 1 and pits[op_pits[pit]] > 0 and game_over == False:
                    ...
                    pits[op_pits[pit]] = 0
                    stones = pits[pit]
                    pit = redistribute_stones(player, pit, stones)
            ...
            #free move message if last stone landed in the bowl
...
def end_game():
    ...
    for i in range(1,7):
        player_pits_total += pits[i]
        pits[i] = 0
    #adds remaining stones from pits to bowls
...
#the stone distribution of the board
